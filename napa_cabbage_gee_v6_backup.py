...
Outputs:
    ...
    9. recent USDA CDL cultivated-land fraction
   10. excess-heat and irrigation-deficit diagnostics
...
class Config:
    ...
    # USDA NASS Cropland Data Layer cultivated-land mask.
    # The official 'cultivated' band is available through 2023;
    # we use recent years to represent contemporary agricultural land.
    use_cultivated_mask: bool = True
    cdl_start_year: int = 2018
    cdl_end_year: int = 2023
    min_cultivated_fraction: float = 0.50
...
def build_feasibility_mask(cfg: Config, cultivated_fraction: ee.Image) -> ee.Image:
    ...
    Exclude clearly infeasible land:
      ...
      - insufficient recent cultivated-land fraction
    ...
    if cfg.use_cultivated_mask:
        cultivated_ok = cultivated_fraction.gte(
            cfg.min_cultivated_fraction
        )
    else:
...
def config_for_mode(mode: str) -> Config:
    ...
    if mode == "quick":
        ...
        return replace(
            ...
            robustness_draws=8,
            export_prefix="napa_cabbage_ca_quick_v6",
        )
    ...
    if mode == "full":
        ...
        return replace(
            ...
            export_prefix="napa_cabbage_ca_full_v6",
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="California Napa cabbage suitability model (v6)"
    )
    parser.add_argument(
        "--project",
    ...
    parser.add_argument(
        "--cultivated-threshold",
        type=float,
        default=None,
        help=(
            "Override the minimum recent cultivated fraction. "
            "Example: --cultivated-threshold 0.50"
        ),
    )
    parser.add_argument(
        "--cultivated-sensitivity",
        action="store_true",
        help=(
            "Submit four otherwise-identical runs with cultivated thresholds "
            "0.15, 0.30, 0.50 and 0.70. Robustness is disabled for these "
            "sensitivity runs so the comparison isolates the land-mask choice."
        ),
    )
    ...
    if args.cultivated_sensitivity:
        thresholds = (0.15, 0.30, 0.50, 0.70)
        print(
            "Submitting cultivated-land sensitivity runs for thresholds: "
        ...
        for threshold in thresholds:
            ...
            cfg = replace(
                base_cfg,
                min_cultivated_fraction=threshold,
                run_robustness=False,
                robustness_draws=0,
                export_prefix=(
                    f"napa_cabbage_ca_{args.mode}_cultivated_"
                    f"{pct:02d}_v6"
                ),
            )
            ...
            all_tasks.append(
                (threshold, image_task.id, table_task.id)
            )

        print("\nEarth Engine cultivated-sensitivity tasks started:")
        for threshold, image_id, table_id in all_tasks:
            print(
                f"  C_min={threshold:.2f} | "
    ...
    if args.cultivated_threshold is not None:
        if not (0.0 <= args.cultivated_threshold <= 1.0):
            parser.error("--cultivated-threshold must lie between 0 and 1.")
        cfg = replace(
            cfg,
            min_cultivated_fraction=args.cultivated_threshold,
            export_prefix=(
                f"{cfg.export_prefix}_cultivated_"
                f"{int(round(args.cultivated_threshold * 100)):02d}"
            ),
        )
