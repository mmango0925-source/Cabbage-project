...
research-scale Earth Engine request graphs below client payload limits.
...
def monte_carlo_robustness(
    ...
    percentile_key = f"score_p{int(percentile)}"
    ...
    def one_draw(weight_obj):
        ...
        threshold = ee.Number(p.get(percentile_key))
        return (
...
def config_for_mode(mode: str) -> Config:
    ...
    if mode == "quick":
        ...
        return replace(
            ...
            robustness_draws=8,
            export_prefix="napa_cabbage_ca_quick_v8",
        )
    ...
    if mode == "full":
        ...
        return replace(
            ...
            export_prefix="napa_cabbage_ca_full_v8",
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="California Napa cabbage suitability model (v8)"
    )
    parser.add_argument(
        "--project",
