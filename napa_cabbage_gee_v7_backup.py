...
def summarize_planting_doy(
    ...
    mean_excess_heat = (
        ee.ImageCollection.fromImages(
            [img.select("excess_heat_c") for img in annual_components]
        )
        .mean()
    ...
    mean_deficit = (
        ee.ImageCollection.fromImages(
            [img.select("deficit_mm") for img in annual_components]
        )
        .mean()
    ...
    doy_img = ee.Image.constant(float(doy)).toFloat().rename("planting_doy")
...
def export_to_drive(
    ...
    # Small metadata-only diagnostic: useful if an export type issue occurs.
    try:
        print("Output band types:", result.bandTypes().getInfo())
    except Exception as exc:
        print("Warning: could not inspect output band types:", exc)
...
def config_for_mode(mode: str) -> Config:
    ...
    if mode == "quick":
        ...
        return replace(
            ...
            robustness_draws=8,
            export_prefix="napa_cabbage_ca_quick_v7",
        )
    ...
    if mode == "full":
        ...
        return replace(
            ...
            export_prefix="napa_cabbage_ca_full_v7",
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="California Napa cabbage suitability model (v7)"
    )
    parser.add_argument(
        "--project",
